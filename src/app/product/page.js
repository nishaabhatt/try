...
//         const cartItem = {
//           name: "Runaway Kind Of Girl-Duffel Bag", // Replace with the actual product name
...
//           <div key={index}>
//             <div className="text-[34px] font-semibold mb-2 ">{option.name}</div>
//             <div className="text-lg font-semibold mb-5 pt-5">{option.subname}</div>
//             <div className="flex pt-3">
//                 <p className="text-xl font-semibold line line-through">${option.priceEarlier}.00</p>
//                 <p className="text-xl font-semibold pl-5">${option.pricePast}.00</p>
//                 <p className="ml-auto text-lg font-medium text-green-500">{option.discount}% off</p>
//             </div>
...
//         <div className="product-pricing my-10">{renderPricingOptions()}</div>
...
const ProductDetails = () => {
  ...
  const addToWishlist = (productPrice) => {
    try {
      const wishlistItem = {
        name: "Oxidised Owning My Perfectionist Tendencies Necklace", // Replace with the actual product name
        price: productPrice, // Replace with the actual product price
      ...
      // Add the new wishlistItem to the wishlistItems array
      wishlistItems.push(wishlistItem);
  
      // Save the updated wishlistItems array to localStorage
      localStorage.setItem("wishlistItems", JSON.stringify(wishlistItems));
  
      // Set itemWishlisted to true to change the button text
      setItemWishlisted(true);
  ...
  return (
    <div className="w-full py-10 px-5 md:px-10 lg:px-20 mx-auto">
      <div className="flex flex-col lg:flex-row gap-10">
        {/* left col start */}
        <div className="w-full md:w-auto flex-1 max-w-[800px] mx-auto">
          <ProductDetailsCarousel />
        </div>
        {/* left col end */}
        {/* right col start */}
  ...
  <div className="flex items-center hover:bg-green-400 cursor-pointer">
    ...
    <p>Years warranty</p>
          ...
          <button
            ...
            className={`py-4 mt-10 border w-full font-medium transition-transform border-green-500
             text-black hover:bg-green-500 hover:text-white active:bg-green-950 
          ...
          {/* wishlist button */}
          <button onClick={() => addToWishlist(pricingData[0].pricePast)}
          className={`py-4 border w-full font-medium mt-2 transition-transform active:scale-95 mb-3
           border-green-500 text-black hover:bg-green-500 hover:text-white
            active:bg-green-950 hover:cursor-pointer flex items-center justify-center 
            ${itemWishlisted ? "bg-white text-green ": ""}`}>
             
             {itemWishlisted ? "Added to Wishlist ": "WHISHLIST"}<IoMdHeartEmpty size={20} className="pl-1"/>
